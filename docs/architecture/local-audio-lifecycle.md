...
| Binding API       | 只接受 candidate/track/expected binding ID 的 Main 窄命令。      | 已实现，尚无 UI 写入调用方                    |
| Playable URL      | 播放期临时资源定位，由受控 resolver 生成，不进入 binding。       | planned                                       |

一个 track 最多一个当前 binding。扫描不会自动绑定；Main 候选 session 不持久化，并按 webContents/generation 隔离。Renderer 不接收可组合成文件引用的目录与相对路径，替换/解绑使用 expected binding ID 防止陈旧操作。binding 不保存 Renderer 可用的绝对路径；playable URL 不作为持久化标识。Web 旧模型仍可保存浏览器 `File` 或 `FileSystemFileHandle`，但这些对象停留在 Web adapter 边界内。
