...
export interface LocalDirectoryEntryDescriptor {
  fileName: string;
  relativePath?: string;
}
...
export interface LocalDirectoryEntryAudioCandidate {
  relativePath?: string;
  albumTitle?: string;
  fileName: string;
  fileNameStem: string;
...
export interface LocalDirectoryIgnoredEntry {
  fileName: string;
  relativePath?: string;
  reason: "unsupported_extension";
}

export interface LocalDirectoryEntryScanError {
  fileName: string;
  relativePath?: string;
...
export interface LocalDirectoryEntryScanResult {
  totalFileCount: number;
  candidates: readonly LocalDirectoryEntryAudioCandidate[];
  ignoredFiles: readonly LocalDirectoryIgnoredEntry[];
  errors: readonly LocalDirectoryEntryScanError[];
}
...
export function scanLocalDirectoryEntries(
  entries: Iterable<LocalDirectoryEntryDescriptor>,
  options: LocalDirectoryScanOptions = {}
): LocalDirectoryEntryScanResult {
  const candidates: LocalDirectoryEntryAudioCandidate[] = [];
  const ignoredFiles: LocalDirectoryIgnoredEntry[] = [];
  const errors: LocalDirectoryEntryScanError[] = [];
  let totalFileCount = 0;
  const knownArtistNames = createKnownArtistNames(options);

  for (const entry of entries) {
    totalFileCount += 1;
    const fileExtension = getLocalAudioFileExtension(entry.fileName);

    if (!fileExtension) {
      ignoredFiles.push({
        fileName: entry.fileName,
        ...(entry.relativePath && isValidLocalDirectoryRelativePath(entry.relativePath)
    ...
    const parsedFileName = parseFileName(entry.fileName, knownArtistNames);
    const issues = [...parsedFileName.issues];
    const candidate: LocalDirectoryEntryAudioCandidate = {
      fileName: entry.fileName,
    ...
    } else if (!isValidLocalDirectoryRelativePath(entry.relativePath)) {
